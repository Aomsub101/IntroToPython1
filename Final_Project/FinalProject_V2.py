...
new_path = os.path.join(current_path, 'Final_Project')
os.chdir(new_path)    
...
def create_account() -> None:
    ...
    while len(password) <= 5 or ' ' in password:
        if len(password) <= 5:
            password = g.getpass("\nPassword must contain more than 5 character(it will be invisible): ")
        elif ' ' in password:
            password = g.getpass("\nPassword must not contain a space (it will be invisible): ")
    ...
    while want_to_lend:
        ...
        while True:
            while True:
                try:
                    days = int(
                        input("Enter number of days you want to lend before returning it: "))
                    break
                except ValueError:
                    print("\nPlease enter a number!")
            if days < 0:
